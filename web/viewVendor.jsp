...
<%@page import="org.websparrow.controller.UserLoginController"%>
	  ...
	  <tr><th>Vendor ID</th><th>Vendor Name</th><th>Address</th><th>Phone</th><th>Email</th><th>Delete</th></tr>  
	  
	  <c:forEach var="row" items="${vendor}">
	  <tr>
	  <td>${row.vid}</td>
	  <td>${row.name}</td>
	  <td>${row.address}</td>
	  <td>${row.phone}</td>
	  <td>${row.email}</td>
	  <td><a href="deleteVendor/${row.vid}">Delete</a></td>			
	  </tr>
	  </c:forEach>
	  </table>
	  <br/>
